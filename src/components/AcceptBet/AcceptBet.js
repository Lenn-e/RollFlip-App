...
function AcceptBet() {
  ...
  useEffect(() => {
    const contract = contractInstanceProvider; // Your initialized contract instance
  ...
  }, [contractInstanceProvider]); // Add dependencies as needed
  ...
  useEffect(() => {
    const link = "http://rollflip.xyz/room/" + gameId.toString();
    ...
    <div className="acceptbet-container">
      {gameDetails && gameId ? (
        <>
          ...
          <p className="acceptbet-amount">Bet Amount: {gameDetails.betAmount} RLFP</p>
